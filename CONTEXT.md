...
- A Refresh runs automatically on a timer (default six hours, user-configurable), and also after the Mac wakes from sleep — but a wake only actually triggers one if the configured auto-refresh interval has elapsed since the last Refresh, so a laptop sleeping/waking many times a day doesn't refresh far more often than the user's chosen interval
